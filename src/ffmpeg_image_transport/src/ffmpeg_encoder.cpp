...
void FFMPEGEncoder::closeCodec() {
  ...
  if (packet_ != nullptr && packet_->data != nullptr) {
...
bool FFMPEGEncoder::openCodec(int width, int height) {
  ...
  try {
    ...
    ROS_DEBUG("codec: %10s, profile: %10s, preset: %10s,"
              ...
              codecName_.c_str(), profile_.c_str(), preset_.c_str(), bitRate_,
              qmax_);
    ...
    */
    ...
    packet_->data = nullptr; // packet data will be allocated by the encoder
    packet_->size = 0;
  ...
  ROS_DEBUG_STREAM("intialized codec " << codecName_ << " for image: " << width
                                       << "x" << height);
  return (true);
}
...
void FFMPEGEncoder::encodeImage(const cv::Mat &img,
  ...
  if (codecName_ == "mjpeg") {
    // For MJPEG, we have to set the quality on every frame.
    frame_->quality = codecContext_->global_quality;
...
int FFMPEGEncoder::drainPacket(const std_msgs::Header &header, int width,
                               int height) {
...
void FFMPEGEncoder::printTimers(const std::string &prefix) const {
  Lock lock(mutex_);
  ROS_INFO_STREAM(
      prefix << " pktsz: " << totalOutBytes_ / frameCnt_
             << " compr: " << totalInBytes_ / (double)totalOutBytes_
             << " debay: " << tdiffDebayer_ << " fmcp: " << tdiffFrameCopy_
             << " send: " << tdiffSendFrame_ << " recv: " << tdiffReceivePacket_
             << " cout: " << tdiffCopyOut_ << " publ: " << tdiffPublish_
             << " tot: " << tdiffTotal_);
}
