...
/**
 * @brief Creates name for the output file based on the input topic name.
 * @param topicName The input topic name.
 * @return The base name for the output file.
 */
std::string fileNameFromTopic(const std::string &topicName) {
  // Replace all '/' characters with '_'
  std::string modified = topicName;
  std::replace(modified.begin(), modified.end(), '/', '_');

  // Strip leading slashes
  while (!modified.empty() && modified.front() == '_') {
    modified.erase(modified.begin());
  }

  // Strip suffix "_ffmpeg" if it exists
  const std::string kRemoveSuffix("_ffmpeg");
  if (modified.size() >= kRemoveSuffix.size() &&
      modified.compare(modified.size() - kRemoveSuffix.size(),
                       kRemoveSuffix.size(), kRemoveSuffix) == 0) {
    modified.erase(modified.size() - kRemoveSuffix.size());
  }
...
using boost::irange;
SplitBag::Session::Session(std::string topic, const std::string &base,
                           bool writeTimeStamp, unsigned int idx)
    : topic_(std::move(topic)), camera_index_(idx), output_base_name_(base) {
  std::string tsname = base + fileNameFromTopic(topic_) + "_ts.txt";
...
void SplitBag::Session::process(const FFMPEGPacketConstPtr &msg) {
  ...
  auto &rawStream = openRawStream(msg);
  if (!rawStream.write((const char *)(&msg->data[0]), msg->data.size())) {
...
std::ofstream &
SplitBag::Session::openRawStream(const SplitBag::FFMPEGPacketConstPtr &msg) {
  if (rawStream_.is_open()) {
    // Already open.
    return rawStream_;
  }

  // Determine the correct extension based on the codec.
  std::string extension;
  if (msg->encoding.find("h264") != std::string::npos) {
    extension = ".h264";
  } else if (msg->encoding.find("h265") != std::string::npos) {
    extension = ".h265";
  } else if (msg->encoding.find("mjpeg") != std::string::npos) {
    extension = ".mjpeg";
  } else {
    ROS_FATAL_STREAM("Unsupported codec found: " << msg->encoding);
    throw std::runtime_error("Unsupported codec found.");
  }

  // Open the file.
  const std::string fname =
      output_base_name_ + fileNameFromTopic(topic_) + extension;
...
void SplitBag::convertToMP4() const {
  for (const auto &idx : irange(0ul, imageTopics_.size())) {
    ...
    std::stringstream ss;
    ss << "ffmpeg -y -r " << videoRate_ << " -i " << iname << " -c:v copy "
       << oname;
...
void SplitBag::processBag(const std::string &fname, double start_time,
                          double duration) {
  ...
  for (const rosbag::MessageInstance &m : view) {
    ...
    if (cnt++ > perfInterval) {
      ...
      ROS_INFO_STREAM("wrote frames: " << frameNum_ << " fps: "
                                       << perfInterval / (imageTopics_.size() *
                                                          (t1 - t0).toSec()));
      cnt = 0;
      t0 = t1;
    }
