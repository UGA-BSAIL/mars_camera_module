# This file was sourced from here:
...
class HailoAsyncInference:
    ...
    def callback(
        ...
        Args:
            ...
            headers (list): The ROS headers corresponding to the input frames.
        """
        if completion_info.exception:
            rospy.logerror(f"Inference error: {completion_info.exception}")
        else:
            for i, bindings in enumerate(bindings_list):
                ...
                try:
                    self.output_queue.put(
                        (input_batch[i], headers[i], result), block=False
                    )
                except queue.Full:
                    rospy.logwarn("Dropping detections due to full queue.")
    ...
    def get_output_names(self) -> List[str]:
        """
        Returns:
            A list of the names of the model outputs, in order.

        """
        output_vstream_info = self.hef.get_output_vstream_infos()
        return [info.name for info in output_vstream_info]

    def get_output_shapes(self) -> List[Tuple[int, ...]]:
        """
        Gets the shape of the model's output layers.

        Returns:
            A list of the model output shapes, in order.

        """
        output_vstream_info = self.hef.get_output_vstream_infos()
        return [info.shape for info in output_vstream_info]
        ...
        with self.infer_model.configure() as configured_infer_model:
            ...
            while True:
                ...
                if self.send_original_frame:
                    original_batch, preprocessed_batch, headers = batch_data
                else:
                    preprocessed_batch, headers = batch_data

                bindings_list = []
                for frame in preprocessed_batch:
                    input_buffer = bindings.input().get_buffer()
                    if input_buffer is None:
                        input_buffer = np.empty_like(frame)
