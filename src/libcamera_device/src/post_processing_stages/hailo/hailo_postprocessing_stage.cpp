...
int HailoPostProcessingStage::configureHailoRT() {
  LOG(1, "Configuring HAILO device");
  ...
  for (const auto &output : infer_model_->outputs()) {
    if (!output.is_nms()) {
      LOG(1, "Configuring output " << output.name() << " to float32.");
  ...
  if (!configured_infer_model_exp) {
    LOG_ERROR("Failed to create configured infer model, status = "
...
hailo_status HailoPostProcessingStage::DispatchJob(
  ...
  if (inf_time_exp && last_frame_.time_since_epoch() != 0s) {
    ...
    if (frame_time < inf_time)
      LOG(0, "Warning: model inferencing time of "
                 << inf_time.count() << "ms "
