...
void Allocator::Reset() {
  ...
  for (auto& info : alloc_info_) munmap(info.ptr, info.size);
...
std::shared_ptr<uint8_t> Allocator::Allocate(unsigned int size) {
  ...
  auto info = std::find_if(
      alloc_info_.begin(), alloc_info_.end(),
  ...
  if (!ptr) {
    void* addr = mmap(NULL, size, PROT_WRITE | PROT_READ,
  ...
  return std::shared_ptr<uint8_t>(ptr,
                                  [this](uint8_t* ptr) { this->free(ptr); });
}
...
void Allocator::free(uint8_t* ptr) {
  ...
  auto info =
      std::find_if(alloc_info_.begin(), alloc_info_.end(),
                   [ptr](const AllocInfo& info) { return info.ptr == ptr; });
  if (info != alloc_info_.end()) info->free = true;
}
...
HailoPostProcessingStage::~HailoPostProcessingStage() {
  if (init_) configured_infer_model_->shutdown();
}
...
void HailoPostProcessingStage::Configure() {
  ...
  if (low_res_stream_) low_res_info_ = app_->GetStreamInfo(low_res_stream_);
  if (output_stream_) output_stream_info_ = app_->GetStreamInfo(output_stream_);

  if (!init_) {
    if (!configureHailoRT()) init_ = true;
  }
...
int HailoPostProcessingStage::configureHailoRT() {
  ...
  Expected<std::shared_ptr<InferModel>> infer_model_exp =
      vdevice_->create_infer_model(hef_file_);
  if (!infer_model_exp) {
    ROS_ERROR_STREAM(
        "Failed to create infer model, status = " << infer_model_exp.status());
  ...
  for (const auto& output : infer_model_->outputs()) {
    if (!output.is_nms()) {
      ROS_INFO_STREAM("Configuring output " << output.name() << " to float32.");
      auto mutable_output = infer_model_->output(output.name()).release();
      mutable_output.set_format_type(HAILO_FORMAT_TYPE_FLOAT32);
    }
  }
  Expected<ConfiguredInferModel> configured_infer_model_exp =
      infer_model_->configure();
  ...
  configured_infer_model_ = std::make_shared<ConfiguredInferModel>(
      configured_infer_model_exp.release());

  // Create infer bindings
  Expected<ConfiguredInferModel::Bindings> bindings_exp =
      configured_infer_model_->create_bindings();
  if (!bindings_exp) {
    ROS_ERROR_STREAM(
        "Failed to create infer bindings, status = " << bindings_exp.status());
...
hailo_status HailoPostProcessingStage::DispatchJob(
    const uint8_t* input, AsyncInferJob& job,
  ...
  status = bindings_.input(input_name)
               ->set_buffer(MemoryView((void*)(input), input_frame_size));
  if (status != HAILO_SUCCESS) {
    ROS_ERROR_STREAM("Could not write to input stream with status " << status);
    return status;
  }
  ...
  for (auto const& output_name : infer_model_->get_output_names()) {
    ...
    status = bindings_.output(output_name)
                 ->set_buffer(MemoryView(output_buffer.get(), output_size));
    if (status != HAILO_SUCCESS) {
      ROS_ERROR_STREAM(
    ...
    const std::vector<hailo_quant_info_t> quant =
        infer_model_->output(output_name)->get_quant_infos();
    const hailo_3d_image_shape_t shape =
        infer_model_->output(output_name)->shape();
    const hailo_format_t format = infer_model_->output(output_name)->format();
    output_tensors.emplace_back(std::move(output_buffer), output_name, quant[0],
                                shape, format);
  }
  ...
  if (status != HAILO_SUCCESS) {
    ROS_ERROR_STREAM("Failed to wait for async ready, status = " << status);
    return status;
  }

  Expected<LatencyMeasurementResult> inf_time_exp =
      configured_infer_model_->get_hw_latency_measurement();
  std::chrono::time_point<std::chrono::steady_clock> this_frame =
      std::chrono::steady_clock::now();

  if (inf_time_exp && last_frame_.time_since_epoch() != 0s) {
    const auto inf_time = std::chrono::duration_cast<std::chrono::milliseconds>(
        inf_time_exp.release().avg_hw_latency);
    const auto frame_time =
        std::chrono::duration_cast<std::chrono::milliseconds>(this_frame -
                                                              last_frame_);

    if (frame_time < inf_time)
      ROS_WARN_STREAM("Warning: model inferencing time of "
                      << inf_time.count() << "ms "
                      << "> current job interval of " << frame_time.count()
                      << "ms!");
  }
  ...
  // Dispatch the job.
  Expected<AsyncInferJob> job_exp =
      configured_infer_model_->run_async(bindings_);
  if (!job_exp) {
    ROS_ERROR_STREAM(
...
HailoROIPtr HailoPostProcessingStage::MakeROI(
    const std::vector<OutTensor>& output_tensors) const {
  HailoROIPtr roi =
      std::make_shared<HailoROI>(HailoROI(HailoBBox(0.0f, 0.0f, 1.0f, 1.0f)));
  ...
  for (auto const& t : output_tensors) {
    ...
    if (HailoRTCommon::is_nms(info)) {
      info.nms_shape =
          infer_model_->output(info.name).release().get_nms_shape().release();
    } else {
      info.shape = t.shape;
    }
...
Rectangle HailoPostProcessingStage::ConvertInferenceCoordinates(
    const std::vector<float>& coords,
    const std::vector<Rectangle>& scaler_crops) const {
  if (coords.size() != 4 || scaler_crops.size() != 2) return {};

  // Convert the inference image co-ordinates into the final ISP output
  // co-ordinates.
  ...
  // Object on the low res scaler crop -> translated by the start of the low res
  // crop offset
  const Rectangle obj_translated_l =
      obj.translatedBy(scaler_crops[1].topLeft());
  ...
  const Rectangle obj_translated_h =
      obj_bounded.translatedBy(-scaler_crops[0].topLeft());
  // -> and scaled to the ISP output.
  const Rectangle obj_scaled =
      obj_translated_h.scaledBy(isp_output_size, scaler_crops[0].size());
