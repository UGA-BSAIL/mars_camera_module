...
/// Maps output types to their sizes in bytes.
const std::unordered_map<hailo_format_type_t, uint8_t> kHailoTypeSizes{
    {HAILO_FORMAT_TYPE_AUTO, 1},
...
/**
 * Extracts the raw data from an output tensor.
...
void ExtractOutputTensorData(const OutTensor &out_tensor,
  ...
  if (kDataSize == kHailoTypeSizes.end()) {
    LOG_ERROR("Unknown format type in output tensor.");
    return;
  }
  const uint32_t kTotalSize = out_tensor.shape.width * out_tensor.shape.height *
                              out_tensor.shape.features * kDataSize->second;
...
YoloInference::~YoloInference() {
  if (yolo_params_) {
    FreeFuncPtr free_func = reinterpret_cast<FreeFuncPtr>(
        postproc_nms_.GetSymbol("free_resources"));
    if (free_func) free_func(yolo_params_);
  }
}
...
void YoloInference::Read(boost::property_tree::ptree const &params) {
  ...
  const std::string config_file =
      params.get<std::string>("hailopp_config_file", {});
  if (!config_file.empty()) {
    if (!fs::exists(config_file))
      throw std::runtime_error(
          std::string("hailo postprocess config file not found: ") +
          config_file);
  }
...
bool YoloInference::Process(CompletedRequestPtr &completed_request) {
  ...
  if (low_res_info_.pixel_format == libcamera::formats::YUV420) {
    ...
    // If the stride shows we have padding on the right edge of the buffer, we
    // must copy it out to another buffer without padding.
    if (low_res_info_.stride != stride) {
      ...
      for (unsigned int i = 0; i < low_res_info_.height; i++)
        memcpy(input_ptr + i * stride, buffer.data() + i * low_res_info_.stride,
               stride);
    } else
  ...
  auto rpi_scaler_crop =
      completed_request->metadata.get(controls::rpi::ScalerCrops);
  ...
  if (objects.size()) {
    if (temporal_filtering_) {
      ...
      // consecutive CompletedRequests if things are running behind.  So protect
      // access to the inference state.
    ...
    if (objects.size())
      completed_request->post_process_metadata.Set("object_detect.results",
                                                   objects);
  }
...
bool YoloInference::runHailoJob(const uint8_t *frame,
  ...
  if (status != HAILO_SUCCESS) {
    LOG_ERROR("Failed to dispatch HAILO job, status = " << status);
    return false;
  }
...
void YoloInference::filterOutputObjects(std::vector<Detection> &objects) {
  ...
  for (auto const &object : objects) {
    ...
    for (auto &lt_obj : lt_objects_) {
      ...
      if (object.category == lt_obj.params.category &&
          ...
          std::abs(object.box.y - lt_obj.params.box.y) <
              tolerance_ * isp_output_size.height &&
          std::abs((int)object.box.width - (int)lt_obj.params.box.width) <
              tolerance_ * isp_output_size.width &&
          std::abs((int)object.box.height - (int)lt_obj.params.box.height) <
              tolerance_ * isp_output_size.height) {
        lt_obj.matched = matched = true;
        lt_obj.params.confidence = object.confidence;
        lt_obj.params.box.x =
            factor_ * object.box.x + (1 - factor_) * lt_obj.params.box.x;
        lt_obj.params.box.y =
            factor_ * object.box.y + (1 - factor_) * lt_obj.params.box.y;
        lt_obj.params.box.width = factor_ * object.box.width +
                                  (1 - factor_) * lt_obj.params.box.width;
        lt_obj.params.box.height = factor_ * object.box.height +
        ...
        // Decrement the hidden counter until the object becomes visible in the
        // list.
    ...
    // Add the object to the long term list if not found.  This object will
    // remain hidden for hidden_frames_ consecutive frames.
    if (!matched)
  ...
  for (auto &lt_obj : lt_objects_) {
    if (!lt_obj.matched) {
      ...
      // consecutive frames before becoming visible. Otherwise, decrement the
      // visible count of unmatched objects in the long term list.
      if (lt_obj.hidden)
        lt_obj.visible = 0;
      else
        lt_obj.visible--;
