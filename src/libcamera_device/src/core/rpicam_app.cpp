...
static libcamera::PixelFormat mode_to_pixel_format(Mode const& mode) {
  // The saving grace here is that we can ignore the Bayer order and return
  // anything - our pipeline handler will give us back the order that works,
  // whilst respecting the bit depth and packing. We may get a "stream adjusted"
  // message, which we can ignore.
...
static void set_pipeline_configuration(Platform platform) {
  ...
  if (existing_config && existing_config[0]) return;

  // Otherwise point it at whichever of these we find first (if any) for the
  // given platform.
  ...
  for (auto& config_file : config_files) {
    struct stat info;
    if (config_file.first == platform &&
        stat(config_file.second.c_str(), &info) == 0) {
...
RPiCamApp::RPiCamApp(std::unique_ptr<Options> opts)
  ...
  if (platform == Platform::LEGACY) {
    // If we definitely appear to be running the old camera stack, complain and
    // give up.
    fprintf(stderr,
    ...
    fprintf(stderr,
            "ERROR: rpicam-apps currently only supports the Raspberry Pi "
            "platforms.\n"
            "Contributions for other platforms are welcome at "
...
RPiCamApp::~RPiCamApp() {
  if (!options_->help)
    LOG(2, "Closing RPiCam application"
               << "(frames displayed " << preview_frames_displayed_
               << ", dropped " << preview_frames_dropped_ << ")");
  StopCamera();
  Teardown();
  CloseCamera();
}
...
std::string const& RPiCamApp::CameraId() const { return camera_->id(); }
...
void RPiCamApp::OpenCamera() {
  ...
  preview_->SetDoneCallback(
      std::bind(&RPiCamApp::previewDoneCallback, this, std::placeholders::_1));

  LOG(2, "Opening camera...");

  if (!camera_manager_) initCameraManager();

  std::vector<std::shared_ptr<libcamera::Camera>> cameras = GetCameras();
  if (cameras.size() == 0) throw std::runtime_error("no cameras available");
  ...
  // We're going to make a list of all the available sensor modes, but we only
  // populate the framerate field if the user has requested a framerate (as this
  // requires us actually to configure the sensor, which is otherwise best
  // avoided).
...
void RPiCamApp::CloseCamera() {
  ...
  if (!options_->help) LOG(2, "Camera closed");
}
...
Mode RPiCamApp::SelectMode(const Mode& mode) const {
  auto scoreFormat = [](double desired, double actual) -> double {
    ...
    if (score < 0.0) score = (-score) / 8;
    // Penalise non-exact matches.
    if (actual != desired) score *= 2;
  ...
  for (const auto& sensor_mode : sensor_modes_) {
    ...
    double fmtAr =
        static_cast<double>(sensor_mode.size.width) / sensor_mode.size.height;
    ...
    if (mode.framerate)
      score +=
          penalty_FPS *
          std::abs(mode.framerate - std::min(sensor_mode.fps, mode.framerate));
  ...
  return {best_mode.size.width, best_mode.size.height, best_mode.depth(),
          mode.packed};
}
...
void RPiCamApp::configureFrameRate(int32_t duration_offset) {
  ...
  if (!controls_.get(controls::FrameDurationLimits)) {
    if (StillStream())
      ...
      int64_t frame_time =
          1000000 / options_->framerate.value_or(DEFAULT_FRAMERATE);  // in us
...
void RPiCamApp::ReConfigureFromOptions() {
  ...
  if (!controls_.get(controls::ScalerCrop) &&
      !controls_.get(controls::rpi::ScalerCrops)) {
    const Rectangle sensor_area =
        camera_->controls().at(&controls::ScalerCrop).max().get<Rectangle>();
    const Rectangle default_crop =
        camera_->controls().at(&controls::ScalerCrop).def().get<Rectangle>();
    ...
    if (options_->lores_width != 0 && options_->lores_height != 0 &&
        !options_->lores_par) {
      crops.push_back(crops.back());
      LOG(2, "Using crop (lores) " << crops.back().toString());
    }

    controls_.set(controls::rpi::ScalerCrops,
                  libcamera::Span<const Rectangle>(crops.data(), crops.size()));
  }

  if (!controls_.get(controls::AfWindows) &&
      !controls_.get(controls::AfMetering) && options_->afWindow_width != 0 &&
  ...
  if (!controls_.get(controls::AfMode) &&
      camera_->controls().count(&controls::AfMode) > 0) {
    int afm = options_->afMode_index;
    if (afm == -1) {
      // Choose a default AF mode based on other options
      if (options_->lens_position || options_->set_default_lens_position ||
  ...
  if (!controls_.get(controls::AfRange) &&
      camera_->controls().count(&controls::AfRange) > 0)
    controls_.set(controls::AfRange, options_->afRange_index);
  if (!controls_.get(controls::AfSpeed) &&
      camera_->controls().count(&controls::AfSpeed) > 0)
    controls_.set(controls::AfSpeed, options_->afSpeed_index);

  if (controls_.get(controls::AfMode).value_or(controls::AfModeManual) ==
      controls::AfModeAuto) {
    ...
    if (!StillStream() && !controls_.get(controls::AfTrigger))
      controls_.set(controls::AfTrigger, controls::AfTriggerStart);
             ...
             camera_->controls().count(&controls::LensPosition) > 0 &&
             !controls_.get(controls::LensPosition)) {
    float f;
  ...
  if (options_->flicker_period && !controls_.get(controls::AeFlickerMode) &&
      camera_->controls().find(&controls::AeFlickerMode) !=
          camera_->controls().end() &&
      camera_->controls().find(&controls::AeFlickerPeriod) !=
          camera_->controls().end()) {
    controls_.set(controls::AeFlickerMode, controls::FlickerManual);
    controls_.set(controls::AeFlickerPeriod,
...
void RPiCamApp::ConfigureViewfinder() {
  ...
  if (have_lores_stream) {
    ...
    configuration_->at(lores_stream_num).bufferCount =
        configuration_->at(0).bufferCount;
  }
  ...
  if (!options_->no_raw) {
    ...
    configuration_->at(raw_stream_num).pixelFormat =
        mode_to_pixel_format(options_->viewfinder_mode);
    configuration_->at(raw_stream_num).bufferCount =
        configuration_->at(0).bufferCount;
    ...
    configuration_->sensorConfig->bitDepth =
        options_->viewfinder_mode.bit_depth;
  }

  configuration_->orientation =
      libcamera::Orientation::Rotate0 * options_->transform;
...
void RPiCamApp::ConfigureZsl(unsigned int still_flags) {
  ...
  if (!options_->no_raw) stream_roles.push_back(StreamRole::Raw);
  ...
  if (options_->width) configuration_->at(0).size.width = options_->width;
  if (options_->height) configuration_->at(0).size.height = options_->height;
  ...
  if (!options_->no_raw) streams_["raw"] = configuration_->at(2).stream();
...
void RPiCamApp::ConfigureStill(unsigned int flags) {
  ...
  // mode, unless the no-raw option is used. (options_->mode can override the
  // choice of camera mode, however.)
  ...
  streams_["still"] = configuration_->at(0).stream();
  if (!options_->no_raw) streams_["raw"] = configuration_->at(1).stream();
...
void RPiCamApp::ConfigureVideo(unsigned int flags) {
  ...
  int lores_index = 1;
  if (!options_->no_raw) stream_roles.push_back(StreamRole::Raw), lores_index++;
  if (have_lores_stream) stream_roles.push_back(StreamRole::Viewfinder);
  configuration_ = camera_->generateConfiguration(stream_roles);
  if (!configuration_)
  ...
  if (options_->buffer_count > 0) cfg.bufferCount = options_->buffer_count;
  if (options_->width) cfg.size.width = options_->width;
  if (options_->height) cfg.size.height = options_->height;
  ...
  if (have_lores_stream) {
    ...
    configuration_->at(lores_index).bufferCount =
        configuration_->at(0).bufferCount;
  }
  configuration_->orientation =
      libcamera::Orientation::Rotate0 * options_->transform;

  configureDenoise(options_->denoise == "auto" ? "cdn_fast"
                                               : options_->denoise);
  setupCapture();
...
void RPiCamApp::Teardown() {
  ...
  for (auto& iter : mapped_buffers_) {
    ...
    for (auto& span : iter.second) munmap(span.data(), span.size());
  }
...
void RPiCamApp::StopCamera() {
  ...
  if (!options_->help) LOG(2, "Camera stopped!");
}

RPiCamApp::Msg RPiCamApp::Wait() { return msg_queue_.Wait(); }
...
void RPiCamApp::queueRequest(CompletedRequest* completed_request) {
  ...
  if (!camera_started_ || !request_found) return;
  ...
  for (auto const& p : buffers) {
    ...
    int ret =
        ::ioctl(p.second->planes()[0].fd.get(), DMA_BUF_IOCTL_SYNC, &dma_sync);
    if (ret)
...
libcamera::Stream* RPiCamApp::GetStream(std::string const& name,
                                        StreamInfo* info) const {
  auto it = streams_.find(name);
  if (it == streams_.end()) return nullptr;
  if (info) *info = GetStreamInfo(it->second);
  return it->second;
}
...
void RPiCamApp::ShowPreview(CompletedRequestPtr& completed_request,
                            Stream* stream) {
  std::lock_guard<std::mutex> lock(preview_item_mutex_);
  if (!preview_item_.stream)
    preview_item_ =
        PreviewItem(completed_request, stream);  // copy the shared_ptr here
  else
...
void RPiCamApp::SetControls(const ControlList& controls) {
  ...
  for (const auto& c : controls) controls_.set(c.first, c.second);
}
...
void RPiCamApp::setupCapture() {
  ...
  for (auto& config : *configuration_) config.stride = 0;
  ...
  for (StreamConfiguration& config : *configuration_) {
    ...
    for (unsigned int i = 0; i < config.bufferCount; i++) {
      ...
      if (!fd.isValid())
        throw std::runtime_error(
            "failed to allocate capture buffers for stream");
      ...
      void* memory = mmap(NULL, config.frameSize, PROT_READ | PROT_WRITE,
                          MAP_SHARED, plane[0].fd.get(), 0);
      mapped_buffers_[fb.back().get()].push_back(libcamera::Span<uint8_t>(
          static_cast<uint8_t*>(memory), config.frameSize));
    }
...
void RPiCamApp::makeRequests() {
  ...
  for (auto& kv : frame_buffers_) {
    ...
    for (auto& b : kv.second) free_buffers[kv.first].push(b.get());
  }
...
void RPiCamApp::requestComplete(Request* request) {
  if (request->status() == Request::RequestCancelled) {
    // If the request is cancelled while the camera is still running, it
    // indicates a hardware timeout. Let the application handle this error.
    if (camera_started_) msg_queue_.Post(Msg(MsgType::Timeout));
  ...
  for (auto const& buffer_map : request->buffers()) {
    ...
    int ret = ::ioctl(buffer_map.second->planes()[0].fd.get(),
                      DMA_BUF_IOCTL_SYNC, &dma_sync);
    if (ret)
  ...
  uint64_t timestamp =
      ts ? *ts : payload->buffers.begin()->second->metadata().timestamp;
...
void RPiCamApp::previewDoneCallback(int fd) {
  ...
  if (it == preview_completed_requests_.end())
    throw std::runtime_error("previewDoneCallback: missing fd " +
                             std::to_string(fd));
  ...
  while (true) {
    ...
    while (!item.stream) {
      ...
      if (preview_abort_) {
        ...
        item =
            std::move(preview_item_);  // re-use existing shared_ptr reference
      else
