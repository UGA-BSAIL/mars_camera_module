...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "filename",
        help="PTS file generated from rpicam-vid (with a .txt or .pts extension)"
        " or an avi/mkv/mp4 container file",
        type=str,
    )
    parser.add_argument("--plot", help="Plot timestamp graph", action="store_true")
    parser.add_argument(
        "--narrow", help="Narrow the y-axis by hiding outliers", action="store_true"
    )
    ...
    print("Outliers:", *[outliers(diffs, f, avg) for f in (1, 0.1, 0.01, 0.001)])
