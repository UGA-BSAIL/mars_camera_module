...
class Commit:
    ...
    def _parse(self):
        ...
        ret = subprocess.run(
            ["git", "show", "--pretty=oneline", "--name-status", self.commit],
    ...
    def get_file(self, filename):
        return subprocess.run(
            ["git", "show", "%s:%s" % (self.commit, filename)], stdout=subprocess.PIPE
        ).stdout.decode("utf-8")
...
class StagedChanges(Commit):
    ...
    def _parse(self):
        ret = subprocess.run(
            ["git", "diff", "--staged", "--name-status"], stdout=subprocess.PIPE
        ).stdout.decode("utf-8")
    ...
    def get_diff(self, top_level, filename):
        diff = subprocess.run(
            ["git", "diff", "--staged", "--", "%s/%s" % (top_level, filename)],
            stdout=subprocess.PIPE,
        ).stdout.decode("utf-8")
        return parse_diff(diff.splitlines(True))
...
class Amendment(StagedChanges):
    ...
    def _parse(self):
        ...
        ret = subprocess.run(
            ["git", "show", "--pretty=oneline", "--no-patch"], stdout=subprocess.PIPE
        ).stdout.decode("utf-8")
        self._title = "Amendment of " + ret.strip()
        # Extract the list of modified files
        ret = subprocess.run(
            ["git", "diff", "--staged", "--name-status", "HEAD~"],
            stdout=subprocess.PIPE,
        ).stdout.decode("utf-8")
        self._files = [CommitFile(f) for f in ret.splitlines()]
    ...
    def get_diff(self, top_level, filename):
        diff = subprocess.run(
            ["git", "diff", "--staged", "HEAD~", "--", "%s/%s" % (top_level, filename)],
            stdout=subprocess.PIPE,
        ).stdout.decode("utf-8")
        return parse_diff(diff.splitlines(True))
...
class ShellChecker(StyleChecker):
    ...
    def check(self, line_numbers):
        ...
        try:
            ret = subprocess.run(
                ["shellcheck", "-Cnever", "-"], input=data, stdout=subprocess.PIPE
            )
        except FileNotFoundError:
            issues.append(
                StyleIssue(
                    0,
                    None,
                    "Please install shellcheck to validate shell script additions",
                )
            )
            return issues
...
class CLangFormatter(Formatter):
    ...
    def format(cls, filename, data):
        ret = subprocess.run(
            ["clang-format", "-style=file", "-assume-filename=" + filename],
...
def check_file(top_level, commit, filename):
    ...
    if len(issues):
        ...
        for issue in issues:
            print(
                "%s#%u: %s" % (Colours.fg(Colours.Yellow), issue.line_number, issue.msg)
            )
...
def check_style(top_level, commit):
    ...
    files = [
        ...
        if len([p for p in patterns if fnmatch.fnmatch(os.path.basename(f), p)])
    ]
    ...
    if issues == 0:
        ...
        print("---")
        print(
            "%u potential %s detected, please review"
...
def extract_commits(revs):
    ...
    """
    ret = subprocess.run(
        ["git", "rev-parse", revs], stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
    if ret.returncode != 0:
